...
void advanced_motor_turn_to_deg(Advanced_Motor* motor, int deg) {
  // turn to degree should only accept a degree value between 0 and 359 inclusive
  if (deg < 0 || deg >= 360) {
    Serial.print("advanced_motor_turn_to_deg was supplied with an invalid degree: ");
    Serial.println(deg);
    return;
  }
  ...
  // these goal needs to be the full number representaion of the degree number that has to be reached.
...
void motor_turn_to_deg(Motor* motor, int deg) {
  ...
  // if goal is lower than current_pos, then we need to choose a new goal, 360 degrees bigger
  if (goal < current_pos) {
...
{
  // HACK: to break, we turn the motor in the other direction for a fix amount of time.
...
void motor_update_degrees(Motor* motor) {
  ...
  if (motor->base.reading) {
    ...
    // we need to add and reset the buffer when we are allowed to write to the degrees
...
void advanced_motor_update_degrees(Advanced_Motor* motor) {
  byte pattern = digitalRead(motor->interrupt_pin1) | (digitalRead(motor->interrupt_pin2) << 1);

  // we determin the direction by a pattern formed by the values read from pin1 and pin2
  // LOW = 0, HIGH = 1
  // 00 and 11 = FORWARD
  // 01 and 10 = BACKWARD
  switch (pattern) {
